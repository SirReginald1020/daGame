import pygame
import torch
import pygame
import torch
from Player import Player
from Platform import Platform
from Camera import Camera
from genetic_algorithm import GABrain  # Import the GA class
from multiprocessing import Pool
import json
import os
import csv


# Constants for grid and screen dimensions
GRID_SIZE = 40  # Grid size for placing platforms
SCREEN_WIDTH, SCREEN_HEIGHT = 800, 600  # Default screen dimensions
FPS = 60  # Frames per second, affects game speed


# Initialize pygame
pygame.init()


# Function to save the game state
def save_game_state():
   """Save both platforms and genetic algorithm state."""
   ga_brain.save_population("population.json")  # Save GA population
   print("Game state saved successfully.")


def update_agent(agent, platforms):
   """Updates the agent's state and calculates fitness."""
   agent.update(platforms)
   fitness = ga_brain.calculate_fitness(agent)
   return (agent, fitness)


# Function to draw a gradient background
def draw_gradient(screen, color_top, color_bottom, width, height):
   """Draw a vertical gradient from color_top to color_bottom."""
   for y in range(height):
       factor = y / height  # Calculate blend factor
       # Interpolate between top and bottom colors
       r = int(color_top[0] + factor * (color_bottom[0] - color_top[0]))
       g = int(color_top[1] + factor * (color_bottom[1] - color_top[1]))
       b = int(color_top[2] + factor * (color_bottom[2] - color_top[2]))
       pygame.draw.line(screen, (r, g, b), (0, y), (width, y))


# Function to create a platform between two points
def create_platform(start_pos, end_pos):
   """Create a platform between start_pos and end_pos."""
   start_pos = snap_to_grid(start_pos, GRID_SIZE)  # Snap to grid
   end_pos = snap_to_grid(end_pos, GRID_SIZE)
   x = min(start_pos[0], end_pos[0])  # Top-left x position
   y = min(start_pos[1], end_pos[1])  # Top-left y position
   width = abs(start_pos[0] - end_pos[0])  # Width of the platform
   height = abs(start_pos[1] - end_pos[1])  # Height of the platform
   return Platform(x, y, width, height)  # Create platform object


# Function to snap a position to the grid
def snap_to_grid(pos, grid_size):
   """Round the position to the nearest grid point."""
   x = round(pos[0] / grid_size) * grid_size
   y = round(pos[1] / grid_size) * grid_size
   return (x, y)


# Function to save platforms to a JSON file
def save_platforms(platforms, filename):
   """Save platforms to a file with the given filename."""
   platform_data = [
       {"x": platform.rect.x, "y": platform.rect.y, "width": platform.rect.width, "height": platform.rect.height}
       for platform in platforms
   ]
   if not os.path.exists("Levels"):
       os.mkdir("Levels")  # Create Levels directory if it doesn't exist
   file_path = os.path.join("Levels", filename + ".json")
   with open(file_path, "w") as file:
       json.dump(platform_data, file)  # Save platform data as JSON
   print(f"Platforms saved successfully as {filename}.json")
   global level_files
   level_files = [f for f in os.listdir("Levels") if f.endswith(".json")]


# Function to save the GA population with a user-defined filename
def save_population_with_prompt(ga_brain, filename_input):
   """Save the GA population with a specified filename."""
   file_path = os.path.join(POPULATIONS_DIR, filename_input + ".json")
   ga_brain.save_population(file_path)  # Save population
   print(f"Population saved successfully as {filename_input}.json")


# Function to list available population files
def load_population_files():
   """Get a list of available population files."""
   return [f for f in os.listdir(POPULATIONS_DIR) if f.endswith(".json")]


# Function to load a GA population from a file
def load_population_from_file(ga_brain, filename):
   """Load the GA population from a specified file."""
   file_path = os.path.join(POPULATIONS_DIR, filename)
   ga_brain.load_population(file_path)
   print(f"Population loaded successfully from {filename}")


# Function to load platforms from a JSON file
def load_platforms_from_file(filename, platforms):
   """Load platforms from a JSON file and add them to the platforms group."""
   with open(filename, "r") as file:
       platform_data = json.load(file)


   platforms.empty()  # Clear existing platforms
   for data in platform_data:
       platform = Platform(data["x"], data["y"], data["width"], data["height"])
       platforms.add(platform)  # Add platform to group
   print(f"Loaded platforms from {filename}")


# Function to draw the pause menu
def draw_menu(screen):
   """Draw the pause menu."""
   screenW = pygame.display.Info().current_w
   screenH = pygame.display.Info().current_h
   font = pygame.font.Font(None, 36)
   options = ["Save Platforms", "Load Level", "Save GA Population", "Load GA Population", "Exit Game"]


   # Draw menu background
   menu_rect = pygame.Surface((screenW, screenH), pygame.SRCALPHA)
   menu_rect.fill((0, 0, 0, 180))  # Black overlay
   screen.blit(menu_rect, (0, 0))


   # Draw menu options
   for i, option in enumerate(options):
       color = (255, 255, 255) if i == selected_option else (150, 150, 150)
       text = font.render(option, True, color)
       screen.blit(text, (screenW // 2 - text.get_width() // 2, screenH // 2 + i * 40 - 20))


# Function to draw the level load menu
def draw_load_menu(screen, level_files, selected_level_index):
   """Draw the load level submenu."""
   screenW = pygame.display.Info().current_w
   screenH = pygame.display.Info().current_h
   font = pygame.font.Font(None, 36)
   load_menu_rect = pygame.Surface((screenW, screenH), pygame.SRCALPHA)
   load_menu_rect.fill((0, 0, 0, 180))
   screen.blit(load_menu_rect, (0, 0))


   # Draw each level file as an option
   for i, filename in enumerate(level_files):
       color = (255, 255, 255) if i == selected_level_index else (150, 150, 150)
       text = font.render(filename, True, color)
       screen.blit(text, (screenW // 2 - text.get_width() // 2, screenH // 2 + i * 40 - 20))


# Function to draw text input for filenames
def draw_text_input(screen, filename_input):
   """Draw the text input box for saving a file."""
   screenW = pygame.display.Info().current_w
   screenH = pygame.display.Info().current_h
   font = pygame.font.Font(None, 36)
   input_box = pygame.Surface((screenW, screenH), pygame.SRCALPHA)
   input_box.fill((0, 0, 0, 180))
   screen.blit(input_box, (0, 0))


   prompt_text = font.render("Enter level name:", True, (255, 255, 255))
   filename_text = font.render(filename_input, True, (255, 255, 255))


   # Center text input on the screen
   screen.blit(prompt_text, (screenW // 2 - prompt_text.get_width() // 2, screenH // 2 - 40))
   screen.blit(filename_text, (screenW // 2 - filename_text.get_width() // 2, screenH // 2))


# Function to draw a grid for alignment purposes
def draw_grid(screen, grid_size, camera, world_width, world_height):
   """Draw a static grid for platform placement and alignment."""
   for x in range(0, world_width, grid_size):
       start_pos = pygame.Rect(x, 0, 1, world_height)
       start_screen_pos = camera.apply(start_pos)
       pygame.draw.line(screen, (200, 200, 200), start_screen_pos.topleft, start_screen_pos.bottomleft)


   for y in range(0, world_height, grid_size):
       start_pos = pygame.Rect(0, y, world_width, 1)
       start_screen_pos = camera.apply(start_pos)
       pygame.draw.line(screen, (200, 200, 200), start_screen_pos.topleft, start_screen_pos.topright)


# Function to get the world position of the mouse
def get_world_position(mouse_pos, camera):
   """Convert screen position to world position considering camera offset."""
   return (
       mouse_pos[0] - camera.camera_rect.topleft[0],
       mouse_pos[1] - camera.camera_rect.topleft[1]
   )


# Function to log the best agent's fitness to a CSV file
def log_best_agent_to_csv(filename, generation, best_fitness, agent_position):
   """Logs the best agent's data (generation, fitness, position) to a CSV file."""
   with open(filename, mode='a', newline='') as file:
       writer = csv.writer(file)
       writer.writerow([generation, best_fitness, agent_position[0], agent_position[1]])


# Setup directories for populations and levels
POPULATIONS_DIR = "Populations"
if not os.path.exists(POPULATIONS_DIR):
   os.mkdir(POPULATIONS_DIR)


# Level and GA setup
if os.path.exists("Levels"):
   level_files = [f for f in os.listdir("Levels") if f.endswith(".json")]
else:
   os.mkdir("Levels")
   level_files = None


defaultLevel = "Mario.json"  # Default level file
defaultLevelPath = "Levels/" + defaultLevel  # Path to the default level


# Initialize genetic algorithm brain for agents
ga_brain = GABrain(
   population_size=10,
   mutation_rate=0.1,
   crossover_rate=0.7,
   platforms_file=defaultLevelPath,
   goal_x=7960  # Target goal position for the GA agents
)


if __name__ == '__main__':
   # Initial menu and interaction states
   is_menu_open = False  # Tracks if the main menu is open
   is_load_menu_open = False  # Tracks if the level load menu is open
   selected_option = 0  # Current selected option in the menu
   selected_level_index = 0  # Current selected level in load menu
   filename_input = ""  # Stores user input for filenames
   is_text_input = False  # Tracks if the text input box is active
   is_population_save_prompt = False  # Tracks if save prompt for population is open
   is_population_load_menu_open = False  # Tracks if population load menu is open
   population_filename_input = ""  # Stores user input for population filename
   selected_population_index = 0  # Selected index in population load menu
   sortedPopulation = []  # Holds sorted population for fitness display


   # Screen setup
   screenInfo = pygame.display.Info()
   screen_width = screenInfo.current_w  # Screen width for fullscreen display
   screen_height = screenInfo.current_h  # Screen height for fullscreen display
   agentFont = pygame.font.Font(None, 24)  # Font for displaying agent info
   screen = pygame.display.set_mode((screen_width, screen_height), pygame.FULLSCREEN)  # Fullscreen mode
   pygame.display.set_caption("daGame")  # Window title


   # Gradient colors for background
   color_top = (186, 223, 255)  # Lighter blue (top color of gradient)
   color_bottom = (104, 183, 252)  # Darker blue (bottom color of gradient)


   # Game timing
   clock = pygame.time.Clock()  # Pygame clock to control FPS


   # Initialize game objects
   player = Player()  # Player object
   generation = 0  # Tracks current generation in genetic algorithm
   camera = Camera(8000, 1000)  # Camera to follow the player and agents
   platforms = pygame.sprite.Group()  # Group to hold all platforms


   # Load default platforms from file
   load_platforms_from_file(defaultLevelPath, platforms)


   # Variables for drawing new platforms
   is_drawing = False  # Indicates if platform drawing mode is active
   start_pos = None  # Start position of the platform being drawn
   current_platform = None  # Currently drawn platform


   # Group for all sprites (including platforms)
   all_sprites = pygame.sprite.Group()
   all_sprites.add(platforms)  # Add platforms to sprite group


   # Default level setup if no levels exist
   if not os.path.exists("Levels"):
       platform_y = camera.height + 100  # Default y-position for platforms
       bottom_platform = Platform(0, platform_y, camera.width, 20)  # Create bottom platform
       platforms.add(bottom_platform)
       platforms.add(Platform(0, 100, 10, camera.height))  # Left boundary platform
       platforms.add(Platform(camera.width - 10, 100, 10, camera.height))  # Right boundary platform


   # Fullscreen state and debugging
   is_fullscreen = True  # Tracks fullscreen mode
   debug = False  # Debug flag for extra logging
   running = True  # Main game loop flag
   frames_per_generation = 1800  # Time interval for each GA generation in frames


   # Main game loop
   while running:
       if debug:
           print(player.rect.x, player.rect.y)  # Debugging player position


       clock.tick(FPS)  # Control game speed to FPS setting
       camera.update(player)  # Update camera to follow player
       player.update(platforms)  # Update player position and behavior


       # Event handling loop
       for event in pygame.event.get():
           if event.type == pygame.QUIT:
               running = False  # Exit game loop on window close
           if event.type == pygame.KEYDOWN:
               if event.key == pygame.K_ESCAPE:  # Escape key toggles menu
                   # Handle menu interactions based on current state
                   if is_text_input:
                       is_text_input = False
                   elif is_load_menu_open:
                       is_load_menu_open = False
                   elif is_population_load_menu_open:
                       is_population_load_menu_open = False
                   else:
                       is_menu_open = not is_menu_open


               elif is_text_input:
                   # Handle text input for saving filenames
                   if event.key == pygame.K_RETURN:
                       save_platforms(platforms, filename_input)
                       filename_input = ""  # Clear input after saving
                       is_text_input = False
                       is_menu_open = False
                   elif event.key == pygame.K_BACKSPACE:
                       filename_input = filename_input[:-1]  # Delete last character
                   elif event.key == pygame.K_SPACE:
                       filename_input += " "
                   else:
                       filename_input += event.unicode  # Add typed character


               # Handling save prompt for GA population
               if is_population_save_prompt:
                   if event.key == pygame.K_RETURN:
                       save_population_with_prompt(ga_brain, population_filename_input)
                       population_filename_input = ""  # Clear after saving
                       is_population_save_prompt = False
                       is_menu_open = False
                   elif event.key == pygame.K_BACKSPACE:
                       population_filename_input = population_filename_input[:-1]
                   else:
                       population_filename_input += event.unicode
               elif is_population_load_menu_open:
                   # Navigate and select population file in load menu
                   if event.key == pygame.K_DOWN:
                       selected_population_index = (selected_population_index + 1) % len(population_files)
                   elif event.key == pygame.K_UP:
                       selected_population_index = (selected_population_index - 1) % len(population_files)
                   elif event.key == pygame.K_RETURN:
                       selected_file = population_files[selected_population_index]
                       load_population_from_file(ga_brain, selected_file)
                       is_population_load_menu_open = False
                       is_menu_open = False


               # Navigate and interact with main menu options
               if is_menu_open and not is_load_menu_open:
                   if event.key == pygame.K_DOWN:
                       selected_option = (selected_option + 1) % 5
                   elif event.key == pygame.K_UP:
                       selected_option = (selected_option - 1) % 5
                   elif event.key == pygame.K_RETURN:
                       if selected_option == 0:
                           is_text_input = True  # Enter filename input mode
                       elif selected_option == 1:
                           is_load_menu_open = True
                           selected_level_index = 0  # Reset level selection
                       if selected_option == 2:
                           is_population_save_prompt = True
                       elif selected_option == 3:
                           population_files = load_population_files()
                           selected_population_index = 0
                           is_population_load_menu_open = True
                       elif selected_option == 4:
                           running = False  # Exit game
               elif is_menu_open and is_load_menu_open:
                   # Navigate and select level file
                   if event.key == pygame.K_DOWN:
                       selected_level_index = (selected_level_index + 1) % len(level_files)
                   elif event.key == pygame.K_UP:
                       selected_level_index = (selected_level_index - 1) % len(level_files)
                   elif event.key == pygame.K_RETURN:
                       selected_file = os.path.join("Levels", level_files[selected_level_index])
                       load_platforms_from_file(selected_file, platforms)
                       is_load_menu_open = False  # Close load menu after loading
                       is_menu_open = False  # Close main menu


           # Toggle fullscreen with F11 key
           if event.type == pygame.KEYDOWN and event.key == pygame.K_F11:
               if is_fullscreen:
                   screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))  # Windowed mode
                   is_fullscreen = False
               else:
                   screen = pygame.display.set_mode((screen_width, screen_height), pygame.FULLSCREEN)
                   is_fullscreen = True


           # Handle player jump with space key
           if event.type == pygame.KEYDOWN:
               if event.key == pygame.K_SPACE:
                   player.jump()


           # Start drawing platform with left mouse button
           if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
               start_pos = get_world_position(pygame.mouse.get_pos(), camera)
               draw_start_pos = pygame.mouse.get_pos()
               is_drawing = True


           # Finish drawing platform with left mouse button release
           if event.type == pygame.MOUSEBUTTONUP and event.button == 1 and is_drawing:
               end_pos = get_world_position(pygame.mouse.get_pos(), camera)
               platform = create_platform(start_pos, end_pos)
               platforms.add(platform)
               is_drawing = False
               start_pos = None
               end_pos = None


           # Delete platform with right mouse button click
           if event.type == pygame.MOUSEBUTTONDOWN and event.button == 3:
               mouse_pos = get_world_position(pygame.mouse.get_pos(), camera)
               for platform in platforms:
                   if platform.rect.collidepoint(mouse_pos):
                       platforms.remove(platform)
                       break


       # Draw gradient background
       draw_gradient(screen, color_top, color_bottom, screen_width, screen_height)
       all_sprites.update(platforms)  # Update all sprites (e.g., platforms)
       player.update(platforms)  # Update player


       # Genetic Algorithm Logic - Update and draw agents
       for agent in ga_brain.population:
           agent.update(platforms)
           agent.draw(screen, camera)


       # Sort population by fitness and log best agent every generation
       sorted_population = sorted(
           [(agent, ga_brain.calculate_fitness(agent)) for agent in ga_brain.population],
           key=lambda x: x[1],
           reverse=True
       )


       # Log the best agent every defined generation interval
       if generation % frames_per_generation == 0:
           best_agent, best_fitness = sorted_population[0]
           log_best_agent_to_csv("best_agent_log.csv", generation, best_fitness, (best_agent.rect.x, best_agent.rect.y))


       # Draw platforms, player, and grid
       for platform in platforms:
           platform.draw(screen, camera)
       player.draw(screen, camera)
       draw_grid(screen, GRID_SIZE, camera, world_width=camera.width, world_height=1100)


       # Draw platform preview if in drawing mode
       if is_drawing:
           mouse_pos_world = get_world_position(pygame.mouse.get_pos(), camera)
           snapped_start_pos = snap_to_grid(start_pos, GRID_SIZE)
           snapped_mouse_pos = snap_to_grid(mouse_pos_world, GRID_SIZE)
           preview_rect = pygame.Rect(
               min(snapped_start_pos[0], snapped_mouse_pos[0]),
               min(snapped_start_pos[1], snapped_mouse_pos[1]),
               abs(snapped_mouse_pos[0] - snapped_start_pos[0]),
               abs(snapped_mouse_pos[1] - snapped_start_pos[1]),
           )
           preview_rect_screen = camera.apply(preview_rect)
           pygame.draw.rect(screen, (0, 255, 0), preview_rect_screen, 2)  # Green outline for platform preview


       # Draw menu screens based on the current mode
       if is_menu_open:
           if is_text_input:
               draw_text_input(screen, filename_input)
           elif is_load_menu_open:
               draw_load_menu(screen, level_files, selected_level_index)
           elif is_population_save_prompt:
               draw_text_input(screen, population_filename_input)
           elif is_population_load_menu_open:
               draw_load_menu(screen, population_files, selected_population_index)
           else:
               draw_menu(screen)


       # Display agents' coordinates and fitness scores
       text_x = 15  # Starting x position for text
       text_y = 50  # Starting y position for text
       for index, (agent, fitness) in enumerate(sorted_population):
           agent_coords = (agent.rect.x, agent.rect.y)
           color = (0, 0, 255) if index == 0 else (200, 0, 0)  # Blue for top agent, red for others
           coordinates_text = agentFont.render(
               f"Agent {index + 1} \n Action: {agent.action1}, {agent.action2} \n (x, y): {agent_coords} | Fitness: {fitness:.2f}",
               True, color)
           screen.blit(coordinates_text, (text_x, text_y))
           text_y += 15  # Move down for the next agent


       pygame.display.flip()  # Update the display each frame


   pygame.quit()  # Quit pygame when game loop ends


from Player import Player
from Platform import Platform
from Camera import Camera
from genetic_algorithm import GABrain  # Import the GA class
from multiprocessing import Pool
import json
import os
import csv


# Constants for grid and screen dimensions
GRID_SIZE = 40  # Grid size for placing platforms
SCREEN_WIDTH, SCREEN_HEIGHT = 800, 600  # Default screen dimensions
FPS = 60  # Frames per second, affects game speed


# Initialize pygame
pygame.init()


# Function to save the game state
def save_game_state():
   """Save both platforms and genetic algorithm state."""
   ga_brain.save_population("population.json")  # Save GA population
   print("Game state saved successfully.")


def update_agent(agent, platforms):
   """Updates the agent's state and calculates fitness."""
   agent.update(platforms)
   fitness = ga_brain.calculate_fitness(agent)
   return (agent, fitness)


# Function to draw a gradient background
def draw_gradient(screen, color_top, color_bottom, width, height):
   """Draw a vertical gradient from color_top to color_bottom."""
   for y in range(height):
       factor = y / height  # Calculate blend factor
       # Interpolate between top and bottom colors
       r = int(color_top[0] + factor * (color_bottom[0] - color_top[0]))
       g = int(color_top[1] + factor * (color_bottom[1] - color_top[1]))
       b = int(color_top[2] + factor * (color_bottom[2] - color_top[2]))
       pygame.draw.line(screen, (r, g, b), (0, y), (width, y))


# Function to create a platform between two points
def create_platform(start_pos, end_pos):
   """Create a platform between start_pos and end_pos."""
   start_pos = snap_to_grid(start_pos, GRID_SIZE)  # Snap to grid
   end_pos = snap_to_grid(end_pos, GRID_SIZE)
   x = min(start_pos[0], end_pos[0])  # Top-left x position
   y = min(start_pos[1], end_pos[1])  # Top-left y position
   width = abs(start_pos[0] - end_pos[0])  # Width of the platform
   height = abs(start_pos[1] - end_pos[1])  # Height of the platform
   return Platform(x, y, width, height)  # Create platform object


# Function to snap a position to the grid
def snap_to_grid(pos, grid_size):
   """Round the position to the nearest grid point."""
   x = round(pos[0] / grid_size) * grid_size
   y = round(pos[1] / grid_size) * grid_size
   return (x, y)


# Function to save platforms to a JSON file
def save_platforms(platforms, filename):
   """Save platforms to a file with the given filename."""
   platform_data = [
       {"x": platform.rect.x, "y": platform.rect.y, "width": platform.rect.width, "height": platform.rect.height}
       for platform in platforms
   ]
   if not os.path.exists("Levels"):
       os.mkdir("Levels")  # Create Levels directory if it doesn't exist
   file_path = os.path.join("Levels", filename + ".json")
   with open(file_path, "w") as file:
       json.dump(platform_data, file)  # Save platform data as JSON
   print(f"Platforms saved successfully as {filename}.json")
   global level_files
   level_files = [f for f in os.listdir("Levels") if f.endswith(".json")]


# Function to save the GA population with a user-defined filename
def save_population_with_prompt(ga_brain, filename_input):
   """Save the GA population with a specified filename."""
   file_path = os.path.join(POPULATIONS_DIR, filename_input + ".json")
   ga_brain.save_population(file_path)  # Save population
   print(f"Population saved successfully as {filename_input}.json")


# Function to list available population files
def load_population_files():
   """Get a list of available population files."""
   return [f for f in os.listdir(POPULATIONS_DIR) if f.endswith(".json")]


# Function to load a GA population from a file
def load_population_from_file(ga_brain, filename):
   """Load the GA population from a specified file."""
   file_path = os.path.join(POPULATIONS_DIR, filename)
   ga_brain.load_population(file_path)
   print(f"Population loaded successfully from {filename}")


# Function to load platforms from a JSON file
def load_platforms_from_file(filename, platforms):
   """Load platforms from a JSON file and add them to the platforms group."""
   with open(filename, "r") as file:
       platform_data = json.load(file)


   platforms.empty()  # Clear existing platforms
   for data in platform_data:
       platform = Platform(data["x"], data["y"], data["width"], data["height"])
       platforms.add(platform)  # Add platform to group
   print(f"Loaded platforms from {filename}")


# Function to draw the pause menu
def draw_menu(screen):
   """Draw the pause menu."""
   screenW = pygame.display.Info().current_w
   screenH = pygame.display.Info().current_h
   font = pygame.font.Font(None, 36)
   options = ["Save Platforms", "Load Level", "Save GA Population", "Load GA Population", "Exit Game"]


   # Draw menu background
   menu_rect = pygame.Surface((screenW, screenH), pygame.SRCALPHA)
   menu_rect.fill((0, 0, 0, 180))  # Black overlay
   screen.blit(menu_rect, (0, 0))


   # Draw menu options
   for i, option in enumerate(options):
       color = (255, 255, 255) if i == selected_option else (150, 150, 150)
       text = font.render(option, True, color)
       screen.blit(text, (screenW // 2 - text.get_width() // 2, screenH // 2 + i * 40 - 20))


# Function to draw the level load menu
def draw_load_menu(screen, level_files, selected_level_index):
   """Draw the load level submenu."""
   screenW = pygame.display.Info().current_w
   screenH = pygame.display.Info().current_h
   font = pygame.font.Font(None, 36)
   load_menu_rect = pygame.Surface((screenW, screenH), pygame.SRCALPHA)
   load_menu_rect.fill((0, 0, 0, 180))
   screen.blit(load_menu_rect, (0, 0))


   # Draw each level file as an option
   for i, filename in enumerate(level_files):
       color = (255, 255, 255) if i == selected_level_index else (150, 150, 150)
       text = font.render(filename, True, color)
       screen.blit(text, (screenW // 2 - text.get_width() // 2, screenH // 2 + i * 40 - 20))


# Function to draw text input for filenames
def draw_text_input(screen, filename_input):
   """Draw the text input box for saving a file."""
   screenW = pygame.display.Info().current_w
   screenH = pygame.display.Info().current_h
   font = pygame.font.Font(None, 36)
   input_box = pygame.Surface((screenW, screenH), pygame.SRCALPHA)
   input_box.fill((0, 0, 0, 180))
   screen.blit(input_box, (0, 0))


   prompt_text = font.render("Enter level name:", True, (255, 255, 255))
   filename_text = font.render(filename_input, True, (255, 255, 255))


   # Center text input on the screen
   screen.blit(prompt_text, (screenW // 2 - prompt_text.get_width() // 2, screenH // 2 - 40))
   screen.blit(filename_text, (screenW // 2 - filename_text.get_width() // 2, screenH // 2))


# Function to draw a grid for alignment purposes
def draw_grid(screen, grid_size, camera, world_width, world_height):
   """Draw a static grid for platform placement and alignment."""
   for x in range(0, world_width, grid_size):
       start_pos = pygame.Rect(x, 0, 1, world_height)
       start_screen_pos = camera.apply(start_pos)
       pygame.draw.line(screen, (200, 200, 200), start_screen_pos.topleft, start_screen_pos.bottomleft)


   for y in range(0, world_height, grid_size):
       start_pos = pygame.Rect(0, y, world_width, 1)
       start_screen_pos = camera.apply(start_pos)
       pygame.draw.line(screen, (200, 200, 200), start_screen_pos.topleft, start_screen_pos.topright)


# Function to get the world position of the mouse
def get_world_position(mouse_pos, camera):
   """Convert screen position to world position considering camera offset."""
   return (
       mouse_pos[0] - camera.camera_rect.topleft[0],
       mouse_pos[1] - camera.camera_rect.topleft[1]
   )


# Function to log the best agent's fitness to a CSV file
def log_best_agent_to_csv(filename, generation, best_fitness, agent_position):
   """Logs the best agent's data (generation, fitness, position) to a CSV file."""
   with open(filename, mode='a', newline='') as file:
       writer = csv.writer(file)
       writer.writerow([generation, best_fitness, agent_position[0], agent_position[1]])


# Setup directories for populations and levels
POPULATIONS_DIR = "Populations"
if not os.path.exists(POPULATIONS_DIR):
   os.mkdir(POPULATIONS_DIR)


# Level and GA setup
if os.path.exists("Levels"):
   level_files = [f for f in os.listdir("Levels") if f.endswith(".json")]
else:
   os.mkdir("Levels")
   level_files = None


defaultLevel = "Mario.json"  # Default level file
defaultLevelPath = "Levels/" + defaultLevel  # Path to the default level


# Initialize genetic algorithm brain for agents
ga_brain = GABrain(
   population_size=10,
   mutation_rate=0.1,
   crossover_rate=0.7,
   platforms_file=defaultLevelPath,
   goal_x=7960  # Target goal position for the GA agents
)


if __name__ == '__main__':
   # Initial menu and interaction states
   is_menu_open = False  # Tracks if the main menu is open
   is_load_menu_open = False  # Tracks if the level load menu is open
   selected_option = 0  # Current selected option in the menu
   selected_level_index = 0  # Current selected level in load menu
   filename_input = ""  # Stores user input for filenames
   is_text_input = False  # Tracks if the text input box is active
   is_population_save_prompt = False  # Tracks if save prompt for population is open
   is_population_load_menu_open = False  # Tracks if population load menu is open
   population_filename_input = ""  # Stores user input for population filename
   selected_population_index = 0  # Selected index in population load menu
   sortedPopulation = []  # Holds sorted population for fitness display


   # Screen setup
   screenInfo = pygame.display.Info()
   screen_width = screenInfo.current_w  # Screen width for fullscreen display
   screen_height = screenInfo.current_h  # Screen height for fullscreen display
   agentFont = pygame.font.Font(None, 24)  # Font for displaying agent info
   screen = pygame.display.set_mode((screen_width, screen_height), pygame.FULLSCREEN)  # Fullscreen mode
   pygame.display.set_caption("daGame")  # Window title


   # Gradient colors for background
   color_top = (186, 223, 255)  # Lighter blue (top color of gradient)
   color_bottom = (104, 183, 252)  # Darker blue (bottom color of gradient)


   # Game timing
   clock = pygame.time.Clock()  # Pygame clock to control FPS


   # Initialize game objects
   player = Player()  # Player object
   generation = 0  # Tracks current generation in genetic algorithm
   camera = Camera(8000, 1000)  # Camera to follow the player and agents
   platforms = pygame.sprite.Group()  # Group to hold all platforms


   # Load default platforms from file
   load_platforms_from_file(defaultLevelPath, platforms)


   # Variables for drawing new platforms
   is_drawing = False  # Indicates if platform drawing mode is active
   start_pos = None  # Start position of the platform being drawn
   current_platform = None  # Currently drawn platform


   # Group for all sprites (including platforms)
   all_sprites = pygame.sprite.Group()
   all_sprites.add(platforms)  # Add platforms to sprite group


   # Default level setup if no levels exist
   if not os.path.exists("Levels"):
       platform_y = camera.height + 100  # Default y-position for platforms
       bottom_platform = Platform(0, platform_y, camera.width, 20)  # Create bottom platform
       platforms.add(bottom_platform)
       platforms.add(Platform(0, 100, 10, camera.height))  # Left boundary platform
       platforms.add(Platform(camera.width - 10, 100, 10, camera.height))  # Right boundary platform


   # Fullscreen state and debugging
   is_fullscreen = True  # Tracks fullscreen mode
   debug = False  # Debug flag for extra logging
   running = True  # Main game loop flag
   frames_per_generation = 1800  # Time interval for each GA generation in frames


   # Main game loop
   while running:
       if debug:
           print(player.rect.x, player.rect.y)  # Debugging player position


       clock.tick(FPS)  # Control game speed to FPS setting
       camera.update(player)  # Update camera to follow player
       player.update(platforms)  # Update player position and behavior


       # Event handling loop
       for event in pygame.event.get():
           if event.type == pygame.QUIT:
               running = False  # Exit game loop on window close
           if event.type == pygame.KEYDOWN:
               if event.key == pygame.K_ESCAPE:  # Escape key toggles menu
                   # Handle menu interactions based on current state
                   if is_text_input:
                       is_text_input = False
                   elif is_load_menu_open:
                       is_load_menu_open = False
                   elif is_population_load_menu_open:
                       is_population_load_menu_open = False
                   else:
                       is_menu_open = not is_menu_open


               elif is_text_input:
                   # Handle text input for saving filenames
                   if event.key == pygame.K_RETURN:
                       save_platforms(platforms, filename_input)
                       filename_input = ""  # Clear input after saving
                       is_text_input = False
                       is_menu_open = False
                   elif event.key == pygame.K_BACKSPACE:
                       filename_input = filename_input[:-1]  # Delete last character
                   elif event.key == pygame.K_SPACE:
                       filename_input += " "
                   else:
                       filename_input += event.unicode  # Add typed character


               # Handling save prompt for GA population
               if is_population_save_prompt:
                   if event.key == pygame.K_RETURN:
                       save_population_with_prompt(ga_brain, population_filename_input)
                       population_filename_input = ""  # Clear after saving
                       is_population_save_prompt = False
                       is_menu_open = False
                   elif event.key == pygame.K_BACKSPACE:
                       population_filename_input = population_filename_input[:-1]
                   else:
                       population_filename_input += event.unicode
               elif is_population_load_menu_open:
                   # Navigate and select population file in load menu
                   if event.key == pygame.K_DOWN:
                       selected_population_index = (selected_population_index + 1) % len(population_files)
                   elif event.key == pygame.K_UP:
                       selected_population_index = (selected_population_index - 1) % len(population_files)
                   elif event.key == pygame.K_RETURN:
                       selected_file = population_files[selected_population_index]
                       load_population_from_file(ga_brain, selected_file)
                       is_population_load_menu_open = False
                       is_menu_open = False


               # Navigate and interact with main menu options
               if is_menu_open and not is_load_menu_open:
                   if event.key == pygame.K_DOWN:
                       selected_option = (selected_option + 1) % 5
                   elif event.key == pygame.K_UP:
                       selected_option = (selected_option - 1) % 5
                   elif event.key == pygame.K_RETURN:
                       if selected_option == 0:
                           is_text_input = True  # Enter filename input mode
                       elif selected_option == 1:
                           is_load_menu_open = True
                           selected_level_index = 0  # Reset level selection
                       if selected_option == 2:
                           is_population_save_prompt = True
                       elif selected_option == 3:
                           population_files = load_population_files()
                           selected_population_index = 0
                           is_population_load_menu_open = True
                       elif selected_option == 4:
                           running = False  # Exit game
               elif is_menu_open and is_load_menu_open:
                   # Navigate and select level file
                   if event.key == pygame.K_DOWN:
                       selected_level_index = (selected_level_index + 1) % len(level_files)
                   elif event.key == pygame.K_UP:
                       selected_level_index = (selected_level_index - 1) % len(level_files)
                   elif event.key == pygame.K_RETURN:
                       selected_file = os.path.join("Levels", level_files[selected_level_index])
                       load_platforms_from_file(selected_file, platforms)
                       is_load_menu_open = False  # Close load menu after loading
                       is_menu_open = False  # Close main menu


           # Toggle fullscreen with F11 key
           if event.type == pygame.KEYDOWN and event.key == pygame.K_F11:
               if is_fullscreen:
                   screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))  # Windowed mode
                   is_fullscreen = False
               else:
                   screen = pygame.display.set_mode((screen_width, screen_height), pygame.FULLSCREEN)
                   is_fullscreen = True


           # Handle player jump with space key
           if event.type == pygame.KEYDOWN:
               if event.key == pygame.K_SPACE:
                   player.jump()


           # Start drawing platform with left mouse button
           if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
               start_pos = get_world_position(pygame.mouse.get_pos(), camera)
               draw_start_pos = pygame.mouse.get_pos()
               is_drawing = True


           # Finish drawing platform with left mouse button release
           if event.type == pygame.MOUSEBUTTONUP and event.button == 1 and is_drawing:
               end_pos = get_world_position(pygame.mouse.get_pos(), camera)
               platform = create_platform(start_pos, end_pos)
               platforms.add(platform)
               is_drawing = False
               start_pos = None
               end_pos = None


           # Delete platform with right mouse button click
           if event.type == pygame.MOUSEBUTTONDOWN and event.button == 3:
               mouse_pos = get_world_position(pygame.mouse.get_pos(), camera)
               for platform in platforms:
                   if platform.rect.collidepoint(mouse_pos):
                       platforms.remove(platform)
                       break


       # Draw gradient background
       draw_gradient(screen, color_top, color_bottom, screen_width, screen_height)
       all_sprites.update(platforms)  # Update all sprites (e.g., platforms)
       player.update(platforms)  # Update player


       # Genetic Algorithm Logic - Update and draw agents
       for agent in ga_brain.population:
           agent.update(platforms)
           agent.draw(screen, camera)


       # Sort population by fitness and log best agent every generation
       sorted_population = sorted(
           [(agent, ga_brain.calculate_fitness(agent)) for agent in ga_brain.population],
           key=lambda x: x[1],
           reverse=True
       )


       # Log the best agent every defined generation interval
       if generation % frames_per_generation == 0:
           best_agent, best_fitness = sorted_population[0]
           log_best_agent_to_csv("best_agent_log.csv", generation, best_fitness, (best_agent.rect.x, best_agent.rect.y))


       # Draw platforms, player, and grid
       for platform in platforms:
           platform.draw(screen, camera)
       player.draw(screen, camera)
       draw_grid(screen, GRID_SIZE, camera, world_width=camera.width, world_height=1100)


       # Draw platform preview if in drawing mode
       if is_drawing:
           mouse_pos_world = get_world_position(pygame.mouse.get_pos(), camera)
           snapped_start_pos = snap_to_grid(start_pos, GRID_SIZE)
           snapped_mouse_pos = snap_to_grid(mouse_pos_world, GRID_SIZE)
           preview_rect = pygame.Rect(
               min(snapped_start_pos[0], snapped_mouse_pos[0]),
               min(snapped_start_pos[1], snapped_mouse_pos[1]),
               abs(snapped_mouse_pos[0] - snapped_start_pos[0]),
               abs(snapped_mouse_pos[1] - snapped_start_pos[1]),
           )
           preview_rect_screen = camera.apply(preview_rect)
           pygame.draw.rect(screen, (0, 255, 0), preview_rect_screen, 2)  # Green outline for platform preview


       # Draw menu screens based on the current mode
       if is_menu_open:
           if is_text_input:
               draw_text_input(screen, filename_input)
           elif is_load_menu_open:
               draw_load_menu(screen, level_files, selected_level_index)
           elif is_population_save_prompt:
               draw_text_input(screen, population_filename_input)
           elif is_population_load_menu_open:
               draw_load_menu(screen, population_files, selected_population_index)
           else:
               draw_menu(screen)


       # Display agents' coordinates and fitness scores
       text_x = 15  # Starting x position for text
       text_y = 50  # Starting y position for text
       for index, (agent, fitness) in enumerate(sorted_population):
           agent_coords = (agent.rect.x, agent.rect.y)
           color = (0, 0, 255) if index == 0 else (200, 0, 0)  # Blue for top agent, red for others
           coordinates_text = agentFont.render(
               f"Agent {index + 1} \n Action: {agent.action1}, {agent.action2} \n (x, y): {agent_coords} | Fitness: {fitness:.2f}",
               True, color)
           screen.blit(coordinates_text, (text_x, text_y))
           text_y += 15  # Move down for the next agent


       pygame.display.flip()  # Update the display each frame


   pygame.quit()  # Quit pygame when game loop ends


